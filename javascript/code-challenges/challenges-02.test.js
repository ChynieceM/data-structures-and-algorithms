...
const raisedToTheThird = (arr) => {
  // Solution code here...
  let newArray = []; //declaring the new array
  arr.forEach(element => { //using for each method on array input array
    newArray.push(Math.pow(element, 3));
  });
  return newArray; //returns new array with each input raised to 3rd power
};
...
const addOne = (arr) => {
  // Solution code here...
  return arr.map(element => element + 1); //returning mapped array incremented by 1
};
...
}; //returning mapped array with each element followed by a ?
...
}; //looping over each item in the array, and pushing the inputs to the power of 2 into the new array and returning the new array
...
}; //doing the same as above but using a different method
...
}; //doing the same as above challenge using a different method.
...
}; //couldnt tell if this works but returning a mapped array that loops through the input array and return the each elements char code.

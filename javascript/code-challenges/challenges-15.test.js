...
let biggerThanLuke = (arr) => {
  // Solution code here...
  let luke = arr.find(character => character.name.toLowerCase() === 'luke skywalker');

  // If Luke Skywalker is not found, just return an empty string
  if (!luke) {
    return '';
  }
  ...
  // Filter characters with mass greater than Luke's
  let biggerCharacters = arr.filter(character => parseInt(character.mass) > lukeMass);

  // Create a string with the names of the bigger characters separated by a dash
  let biggerNames = biggerCharacters.map(character => character.name).join(' - ');
...
const sortBy = (property, arr) => {
  ...
  return arr.sort((a, b) => {
    if (typeof a[property] === 'number' && typeof b[property] === 'number') {
      ...
      return a[property].toString().toLowerCase().localeCompare(b[property].toString().toLowerCase());
    }
  });
};
...
const detectTicTacToeWin = (board) => {
  // Solution code here...
  const checkCells = (a, b, c) => a === b && b === c && a !== '';
  ...
  if (checkCells(board[0][2], board[1][1], board[2][0])) {
    return true;
  }

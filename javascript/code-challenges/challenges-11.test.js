...
const count = (target, input) => {
  // Solution code here...
  return input.reduce((acc, currentArr) => {
    const spotInArray = currentArr.reduce((accTwo, currentVal) => {
...
const totalSum = (input) => {
  // Solution code here...
  return input.reduce((acc, currentArr) => {
    const sumCurrentOfArr = currentArr.reduce((accTwo, currentVal) => {
      return accTwo + currentVal;
    }, 0);
    return acc + sumCurrentOfArr;
  }, 0);
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  return input.map((currentArr) => {
    const filteredArray = currentArr.filter((element) => {
      return typeof element === 'number' && element % 5 === 0;
    });
    return filteredArray.map((num) => {
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  let maleFemaleCharacters = data.filter(character => {
    return character.gender === 'male' || character.gender === 'female';
  });

  let names = maleFemaleCharacters.map(character => character.name);
